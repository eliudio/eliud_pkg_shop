...
/*
    ...
    if (currentState is PayUninitialised) {
*/
    if (event is CollectOrder) {
      // The payment screen is opened. We create an OrderModel instance in membory
        if (!GlobalData.isLoggedOn()) {
          yield NotLoggedOn();
          return;
        } else if (!CartTools.hasItems(GlobalData.app(), GlobalData.member())) {
          yield NoItemsInCart();
          return;
        } else {
          yield ConfirmOrder(_getNewOrder(event.shop));
          return;
        }
  ...
  OrderModel _getNewOrder(ShopModel shop) {
    return OrderModel(
        ...
        products: GlobalData
            ...
            .toList(),
        totalPrice: CartTools.totalValue());
  }
}
