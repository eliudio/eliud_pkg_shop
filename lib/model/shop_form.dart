  ...
  Widget build(BuildContext context) {
    ...
    if (formAction == FormAction.ShowData) {
      return BlocProvider<ShopFormBloc >(
            create: (context) => ShopFormBloc(AccessBloc.appId(context),
                                       formAction: formAction,
      ...
      return Scaffold(
        appBar: StyleRegistry.registry().styleWithContext(context).adminFormStyle().constructAppBar(context, formAction == FormAction.UpdateAction ? 'Update Shop' : 'Add Shop'),
    ...
    return BlocBuilder<ShopFormBloc, ShopFormState>(builder: (context, state) {
      ...
      if (state is ShopFormInitialized) {
        ...
        children.add(
                  ...
                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Document ID', Icons.vpn_key, (formAction == FormAction.UpdateAction), _documentIDController, FieldType.String, validator: (_) => state is DocumentIDShopFormError ? state.message : null, hintText: 'null')
          );

        children.add(

                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'description', Icons.text_format, _readOnly(accessState, state), _descriptionController, FieldType.String, validator: (_) => state is DescriptionShopFormError ? state.message : null, hintText: 'null')
          );

        children.add(

                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Short Description', Icons.text_format, _readOnly(accessState, state), _shortDescriptionController, FieldType.String, validator: (_) => state is ShortDescriptionShopFormError ? state.message : null, hintText: 'null')
          );

        children.add(

                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Currency', Icons.text_format, _readOnly(accessState, state), _currencyController, FieldType.String, validator: (_) => state is CurrencyShopFormError ? state.message : null, hintText: 'null')
          );
