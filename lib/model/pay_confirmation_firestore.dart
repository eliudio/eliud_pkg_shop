...
class PayConfirmationFirestore implements PayConfirmationRepository {
  ...
  Future<PayConfirmationModel?> get(String? id, {Function(Exception)? onError}) {
    return PayConfirmationCollection.doc(id).get().then((doc) async {
      if (doc.data() != null)
...
//    In the meantime:
      ...
      return payConfirmations as List<PayConfirmationModel?>;
    });
...
//  see comment listen(...) above
  ...
  Stream<List<PayConfirmationModel?>> values({String? orderBy, bool? descending, Object? startAfter, int? limit, SetLastDoc? setLastDoc, int? privilegeLevel, EliudQuery? eliudQuery }) {
    ...
    Stream<List<PayConfirmationModel?>> _values = getQuery(PayConfirmationCollection, orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?, limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots().map((snapshot) {
